...
export default function SummaryCheckout({ customerInfo }) {
    ...
    const { total_sin_iva, iva, total_recargo, total_factura } = calculateTotals({vatRate: vatRate, isRecharge: customerInfo.isRecharge});
    ...
    return (
        <div className="flex justify-end m-20 px-20">
            <div className="flex flex-col gap-2 w-1/5">
                ...
                <div className="flex text-lg font-bold justify-between">{t('invoice.total.total')}: <span className="flex">{total_factura.toFixed(2)} €</span></div>
            </div>
        </div>
    );
}